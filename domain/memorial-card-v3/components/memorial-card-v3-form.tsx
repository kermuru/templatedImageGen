...
import type { MemorialCardV3Data } from "../type";

export function MemorialCardV3Form() {
  const [data, setData] = useState<MemorialCardV3Data>({
    full_name: "",
    date_of_interment: "",
    mass_service_time: "",
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/memorial-card-v3/upload", { method: "POST", body: form });
  ...
  function set(field: keyof MemorialCardV3Data, value: string) {
  ...
  function buildParams() {
    ...
    if (data.location_sub) params.set("location_sub", data.location_sub);
    if (data.time_caption) params.set("time_caption", data.time_caption);
  ...
  function handleGenerate() {
    setGeneratedUrl(`${window.location.origin}/api/memorial-card-v3/image?${buildParams()}&t=${Date.now()}`);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-amber-400">Memorial Card Generator — V3 Green</h1>
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="flex-1 max-w-xl bg-slate-900 rounded-2xl p-6 space-y-5">
            ...
            {/* Venue — drawn by V3 rather than baked, so another chapel needs no
                new template. Blank falls back to the artwork's defaults. */}
            <div>
              <label className="block text-sm text-slate-400 mb-1">
                Location <span className="text-slate-600 text-xs">(bold line beside the pin)</span>
              </label>
              <input type="text" value={data.location ?? ""} onChange={(e) => set("location", e.target.value)}
                placeholder="RENAISSANCE PARK" className={inputClass} />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-slate-400 mb-1">
                  Location caption <span className="text-slate-600 text-xs">(under it)</span>
                </label>
                <input type="text" value={data.location_sub ?? ""} onChange={(e) => set("location_sub", e.target.value)}
                  placeholder="PARK AND CHAPELS" className={inputClass} />
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">
                  Time caption <span className="text-slate-600 text-xs">(under the time)</span>
                </label>
                <input type="text" value={data.time_caption ?? ""} onChange={(e) => set("time_caption", e.target.value)}
                  placeholder="AT RENAISSANCE PARK" className={inputClass} />
              </div>
            </div>
