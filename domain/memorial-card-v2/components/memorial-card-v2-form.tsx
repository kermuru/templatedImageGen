...
import type { MemorialCardV2Data } from "../type";

export function MemorialCardV2Form() {
  const [data, setData] = useState<MemorialCardV2Data>({
    full_name: "",
    date_of_interment: "",
    mass_service_time: "",
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/memorial-card-v2/upload", { method: "POST", body: form });
  ...
  function set(field: keyof MemorialCardV2Data, value: string) {
  ...
  function handleGenerate() {
    setGeneratedUrl(`${window.location.origin}/api/memorial-card-v2/image?${buildParams()}&t=${Date.now()}`);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-amber-400">Memorial Card Generator — V2</h1>
          <p className="text-slate-400 mt-1">
            Portrait-in-arch template. Fill in the details and generate a shareable memorial card image.
          </p>
        </div>
          ...
          <div className="flex flex-col gap-4 flex-1">
            ...
            {generatedUrl ? (
              ...
              <div className="flex items-center justify-center rounded-xl bg-slate-900 text-slate-600 text-sm"
                ...
                Fill in the details and click &quot;Generate Memorial Card V2&quot;
              </div>
            )}
          </div>
