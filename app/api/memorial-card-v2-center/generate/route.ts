...
import { GET } from "@/app/api/memorial-card-v2-center/image/route";
...
export async function POST(request: NextRequest) {
  ...
  if (photo_url) {
    try {
      ...
      console.error("[memorial-card-v2-center/generate] photo download failed:", err);
    }
  }

  const imageUrl = new URL("/api/memorial-card-v2-center/image", request.url);
  ...
  const filename = `memorial-v2-center/${crypto.randomUUID()}.png`;
