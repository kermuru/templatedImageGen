 ...
 * arch. Returns a transparent PNG buffer, or the original bytes if the key is
 * missing / the call fails (non-fatal).
 */
async function stripBackground(bytes: Buffer): Promise<{ buffer: Buffer; removed: boolean }> {
  ...
  try {
    ...
    console.error("[memorial-card-v2-center/upload] remove.bg failed:", res.status, await res.text());
  } catch (err) {
    console.error("[memorial-card-v2-center/upload] remove.bg error:", err);
  }
...
export async function POST(request: NextRequest) {
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "memorial-v2-center");

  await mkdir(uploadDir, { recursive: true });
  await writeFile(path.join(uploadDir, filename), buffer);

  return NextResponse.json({ url: `/uploads/memorial-v2-center/${filename}`, bg_removed: removed });
}
