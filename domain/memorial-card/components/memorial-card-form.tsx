...
import type { MemorialCardData } from "../type";

export function MemorialCardForm() {
  const [data, setData] = useState<MemorialCardData>({
    full_name: "",
    date_of_interment: "",
    mass_service_time: "",
  ...
  function set(field: keyof MemorialCardData, value: string) {
  ...
  function handleGenerate() {
    setGeneratedUrl(`${window.location.origin}/api/memorial-card/image?${buildParams()}&t=${Date.now()}`);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-amber-400">Memorial Card Generator</h1>
          <p className="text-slate-400 mt-1">Fill in the details and generate a shareable memorial card image.</p>
        </div>
          ...
          <div className="flex-1 max-w-xl bg-slate-900 rounded-2xl p-6 space-y-5">
            ...
            <div>
              ...
              <input type="text" value={data.full_name} onChange={(e) => set("full_name", e.target.value)}
                placeholder="e.g. Josefina Espiñeda Dela Peña" className={inputClass} />
            </div>
          ...
          <div className="flex flex-col gap-4 flex-1">
            ...
            {generatedUrl ? (
              ...
              <div className="flex items-center justify-center rounded-xl bg-slate-900 text-slate-600 text-sm"
                ...
                Fill in the details and click &quot;Generate Memorial Card&quot;
              </div>
            )}
          </div>
