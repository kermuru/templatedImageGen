...
async function stripBackground(bytes: Buffer): Promise<{ buffer: Buffer; removed: boolean }> {
  ...
  try {
    ...
    console.error("[memorial-card-v3/upload] remove.bg failed:", res.status, await res.text());
  } catch (err) {
    console.error("[memorial-card-v3/upload] remove.bg error:", err);
  }
...
export async function POST(request: NextRequest) {
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "memorial-v3");

  await mkdir(uploadDir, { recursive: true });
  await writeFile(path.join(uploadDir, filename), buffer);

  return NextResponse.json({ url: `/uploads/memorial-v3/${filename}`, bg_removed: removed });
}
